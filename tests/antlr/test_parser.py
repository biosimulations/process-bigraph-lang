...
def test_parse_abc(model_path_abc: Path) -> None:
    ...
    assert processDef_strs == [
        "(processDef process_def MyProcess param \n"
        "  (schemaItem calcium_growth : float default \n"
        "    (defaultValue 0.1) [ uM_per_h ]) param \n"
        "  (schemaItem glucose_growth : float default \n"
        "    (defaultValue 0.1) [ uM_per_h ]) param \n"
        "  (schemaItem dt : float default \n"
        "    (defaultValue 0.1) [ hour ]) param \n"
        "  (schemaItem alpha : float default \n"
        "    (defaultValue 0.1) [ dimensionless ]) var \n"
        "  (schemaItem glucose : float) var \n"
        "  (schemaItem calcium : float [ conc ]) inputs glucose calcium outputs "
        "glucose calcium updates \n"
        "  (update glucose := \n"
        "    (expression \n"
        "      (addition \n"
        "        (multiplication \n"
