...
from process_bigraph_lang.dsl.langium_pblang import validate_pblang_file


def test_validate_abc(model_path_abc: Path) -> None:
    stdout, stderr = validate_pblang_file(model_path_abc)
    assert stderr == ""
...
def test_validate_abc_should_fail(model_path_abc_error: Path) -> None:
    stdout, stderr = validate_pblang_file(model_path_abc_error)
    assert stderr == (
...
def test_validate_add_floats(model_add_floats: Path) -> None:
    stdout, stderr = validate_pblang_file(model_add_floats)
    assert stderr == ""
    assert stdout.startswith("Parsed and validated ")
    assert stdout.endswith("successfully!")
...
def test_validate_dfba_single(model_dfba_single: Path) -> None:
    stdout, stderr = validate_pblang_file(model_dfba_single)
    assert stderr == ""
    assert stdout.startswith("Parsed and validated ")
    assert stdout.endswith("successfully!")
