...
class Particles(Process):
    env_size: tuple[tuple[float, float], tuple[float, float]]  # ((x_min, x_max), (y_min, y_max))
    config_schema = {
    ...
    def initialize(self, config: dict[str, Any]) -> None:
        self.env_size = ((0.0, float(config["bounds"][0])), (0.0, float(config["bounds"][1])))
    ...
    @staticmethod
    def generate_state(config: dict[str, Any] | None = None) -> dict[str, Any]:
        """
        ...
        if len(fields) > 0:
            ...
            if fields_bins != n_bins:
                raise ValueError(f"Shape of fields {fields_bins} does not match n_bins {n_bins}")

        # advection_rates = advection_rates or [(0.0, 0.0) for _ in range(len(n_particles_per_species))]
        particles = {}
        for _ in range(n_particles):
            id = Particles.short_id()
            particles[id] = Particles.generate_single_particle_state(
                config={
                    "bounds": bounds,
    ...
    def update(self, state: dict[str, Any], interval: float) -> dict[str, Any]:
        ...
        new_particles: dict[str, Any] = {"_remove": [], "_add": {}}
        new_fields = {mol_id: np.zeros_like(field) for mol_id, field in fields.items()}

        for particle_id, particle in particles.items():
            updated_particle: dict[str, Any] = {"exchange": {}}
            ...
            x, y = Particles.get_bin_position(new_position, self.config["n_bins"], self.env_size)

            # Update local environment values for each particle
            updated_particle["local"] = Particles.get_local_field_values(fields, column=x, row=y)
        ...
        for boundary in self.config["boundary_to_add"]:
            if np.random.rand() < self.config["add_probability"]:
                ...
                x, y = Particles.get_bin_position(position, self.config["n_bins"], self.env_size)
                local_field_concentrations = Particles.get_local_field_values(fields, column=x, row=y)
                id = Particles.short_id()  # str(uuid.uuid4())
                new_particle = {
                    "id": id,
                    "position": position,
        ...
        return {"particles": new_particles, "fields": new_fields}

    def check_boundary_hit(self, new_x_position: float, new_y_position: float) -> bool:
    ...
    def get_boundary_position(self, boundary: Literal["left", "right", "top", "bottom"]) -> tuple[float | int, float]:
        if boundary == "left":
            ...
            raise ValueError(f"Unknown boundary: {boundary}. Must be one of 'left', 'right', 'top', or 'bottom'.")

    @staticmethod
    def get_bin_position(
        position: tuple[float, float],
        n_bins: tuple[int, int],
        env_size: tuple[tuple[float, float], tuple[float, float]],
    ) -> tuple[int, int]:
        x, y = position
    ...
    @staticmethod
    def get_local_field_values(fields: dict[str, np.ndarray], column: int, row: int) -> dict[str, float]:
        """
    ...
    def generate_single_particle_state(config: ProcessTypes = None) -> dict[str, Any]:
        ...
        position = cast(tuple[float, float], tuple(np.random.uniform(low=[0, 0], high=[bounds[0], bounds[1]], size=2)))
...
class MinimalParticle(Process):
    ...
    def inputs(self) -> dict[str, Any]:
        return {"mass": "float", "substrates": "map[positive_float]"}

    def outputs(self) -> dict[str, Any]:
        return {"mass": "float", "substrates": "map[float]"}
    ...
    def update(self, state: dict[str, Any], interval: float) -> dict[str, Any]:
        ...
        for reaction_name, reaction in self.config["reactions"].items():
            ...
            reaction_rate: float = numerator / denominator
            reaction_rates[reaction_name] = reaction_rate

            total_reactant = 0.0
            for reactant in roles["reactant"]:
                if reactant not in exchanges:
        ...
        update = {"mass": total_reactant, "substrates": exchanges}

        return update

    def large_update(self, state: dict[str, Any], interval: float) -> dict[str, Any]:
        substrates_input = state["substrates"]
        exchanges: dict[str, float] = {}

        # Helper functions for interaction types
        def michaelis_menten(uptake_value: float, vmax: float, Km: float) -> float:
            """Michaelis-Menten rate law for uptake."""
            return (vmax * uptake_value) / (Km + uptake_value) if Km + uptake_value > 0 else 0

        def calculate_uptake(field_value: float, vmax: float, Km: float) -> float:
        ...
        def calculate_secretion(vmax: float) -> float:
...
def get_particles_spec(
    ...
    diffusion_rate: float = 1e-1,
    advection_rate: tuple[float, float] = (0, 0),
    add_probability: float = 0.0,
    boundary_to_add: list[str] = ["top"],
) -> dict[str, Any]:
    config = locals()
...
def get_particles_state(
    ...
    advection_rate: tuple[float, float] = (0, -0.1),
    boundary_to_add: list[str] | None = None,
    ...
    particles = Particles.generate_state(
        config={"n_particles": n_particles, "n_bins": n_bins, "bounds": bounds, "fields": fields}
    )
        ...
        "particles": {
            ...
            "_value": {
                ...
                "minimal_particle": {
                    ...
                    "inputs": default("tree[wires]", {"mass": ["mass"], "substrates": ["local"]}),
                    "outputs": default("tree[wires]", {"mass": ["mass"], "substrates": ["exchange"]}),
                }
            },
        }
    }
