  ...
  connection.onExecuteCommand(async (params: ExecuteCommandParams) => {
    if (params.command === "pblang/generateStub") {
      ...
      connection.window.showInformationMessage(
        result ? "Stub generation succeeded!" : "Stub generation failed.",
      );
    }
  });
}

async function handleGenerateStubCommand(
  args: any[] | undefined,
): Promise<boolean> {
  try {
    ...
    for (const fileUri of pblangFiles) {
      ...
      const stubPath = path.resolve(
        ...
        "generated",
        `${path.basename(filePath, ".sbml_stub")}.pblang`,
      );
      const relativePath = path.relative(path.dirname(stubPath), filePath);
      const stubPblang = generateStubPblang(
        sbmlContent,
        sbmlModelName,
        relativePath,
      );
