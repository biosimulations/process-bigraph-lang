...
export function validateValueAgainstType(value: Value, expected: ResolvedType, accept: ValidationAcceptor): boolean {
    switch (expected.kind) {
        case 'primitive':
            ...
            if (expected.name === 'float') {
                if (!isFloatLiteral(value)) {
                    accept('error', `Expected value of type 'float'`, { node: value });
                    return false;
                }
                return true;
            }
            ...
            accept('error', `Unknown primitive type '${expected.name}'`, { node: value });
            return false;
        case 'struct':
            if (!isStructLiteral(value)) {
                accept('error', `Expected struct of type '${expected.type.name}'`, { node: value });
                return false;
            }
            ...
            for (const f of value.fields) {
                ...
                if (!def) {
                    accept('error', `Field '${f.name}' is not valid for struct '${expected.type.name}'`, { node: f, property: 'name' });
                    structValid = false;
                    continue;
                }
            ...
            for (const fieldName of definedFields.keys()) {
                if (!seenFields.has(fieldName)) {
                    accept('error', `Missing field '${fieldName}' in struct literal`, { node: value });
                    structValid = false;
                }
            }
            return structValid;
        ...
        case 'tuple':
            ...
            let hasError = false;
            const expectedFields = new Map(expected.elements.map(e => [e.name, e.type]));
            const seen = new Set<string>();

            for (const field of value.fields) {
                const name = field.name;
                if (seen.has(name)) {
                    accept('error', `Duplicate tuple field '${name}'`, { node: field, property: 'name' });
                    hasError = true;
                    continue;
                }
                seen.add(name);
                ...
                if (!expectedType) {
                    accept('error', `Field '${name}' is not valid for this tuple`, { node: field, property: 'name' });
                    hasError = true;
                    continue;
                }
        ...
        case 'map':
            ...
            for (const entry of value.entries) {
                const keyValid = validateValueAgainstType(entry.key, expected.keyType, accept);
                const valValid = validateValueAgainstType(entry.value, expected.valueType, accept);
